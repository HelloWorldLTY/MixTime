...
agent = STPathInference(
    gene_voc_path='./utils_data/symbol2ensembl.json',
    model_weight_path='./STPath/stfm.pth', 
    device=0
)
...
random_matrix = np.random.randint(100,size=(10, 2))
...
for i in slide_info:
    ...
    image_info = Image.open(f"/home/tl688/zhao_project/GigaTIME/cscc_info_mixtime/gsminfo_out/{i}_tissue_hires_image.png").convert('RGB')
    file_list = []
    list_info = adata.obsm['X_loc'].astype('int') #stpath requires location information, you acn also save it as (x,y) of patch
    gigapathemb = torch.load(f"/home/tl688/zhao_project/GigaTIME/allemb_cscc/{i}_gigapath.pkl")

    pred_out_list,xout_list = [],[]
    batchsize = 512
    for idx in tqdm.tqdm(range(0, len(np.array(list_info)), batchsize)):
        pred_adata, xout = agent.inference(
            coords=np.array(list_info)[idx:idx+batchsize,:], # [number_of_spots, 2]
            img_features=gigapathemb.numpy()[idx:idx+batchsize,:])

        pred_out_list.append(pred_adata)
        xout_list.append(xout)

    mydict = {}
    mydict['pred'] = np.concatenate(pred_out_list)
    mydict['emb'] = np.concatenate(xout_list)

    import pickle
    with open(f'/home/tl688/zhao_project/GigaTIME/allemb_cscc/{i}_stpath.pkl', 'wb') as f: # Open in write binary mode ('wb')
